...
- 📱 Screens page is organized in 3 sections: **EN · Send money flow** (01 · Home `1100:123`, 02 · AI Chat `1100:273`, 03 · Review Transfer `1104:123`, 04 · Success `1117:123`), **AR · Send money flow (RTL)** (same screens + " — AR", placed right→left), **Archive** (Home v1 `1064:3211`, "curser" `1062:5`, Font Exploration `1096:5`). Sections: 80 padding, 160 top, 120 between screens, 240 between sections. New screens go into the matching section.
...
- Scroll screens: frame stays 390×844 (clip on, vertical overflow); Status Bar + bottom bar/CTA + Home Indicator are fixed (topmost layers); Content paddingBottom = bar height + 24. Screen3_Review `1104:123`, Screen4_Success `1117:123` (EN flow complete). **AR (RTL) row below:** Home — AR `1118:273`, Screen2_Chat — AR `1120:123`, Screen3_Review — AR `1120:215`, Screen4_Success — AR `1120:315`. RTL mirroring = reverse horizontal auto-layout order, swap L/R paddings + asymmetric radii, MIN↔MAX alignment, rotate directional icons 180°, then send `Ring*`/`Decor*` absolute layers back to the bottom. AR currency format: `12,450.00 د.إ`.
...
- **Source of truth = the screens.** Never edit 📱 Screens to match components — update 🧩 Components to match the screens (Sep 2026: all 27 components aligned: buttons pill 40/52/56 + green gradient, icon buttons 32/40/44 circles, AI/Orb 32/40/44, AI/Input 72 r24, Balance Card r24 p20/24, rows 60 / 84 with gap 16, cards r24 + white stroke, nav bottom r32 top). Screens currently use 0 instances.
- **Title Case for EN titles:** section/card titles, page titles and button/link labels use Title Case (minor words like to/for/of stay lowercase, e.g. "What Would You Like to Do?"); body text, subtitles and field labels stay sentence case. Applied Sep 2026 to the EN section `1169:325`.
...
- **🧭 UX Methods page** `1162:5` (docs for the Send money flow, 1760-wide boards in sections, same DS styles/variables): 00 · Overview `1162:6`, 01 · Personas `1162:29` (Kareem Adel — EN, Fatima Al Hashimi — AR), 02 · User Journey Map `1164:7` (6 stages + emotion curve), 03 · User Flow & Edge Cases `1165:5` (happy path + 8 missing screens ranked P1–P3), 04 · Heuristic Evaluation `1167:5` (13 pinned findings + Nielsen scorecard). Screen images there are PNG exports — re-export if screens change.
